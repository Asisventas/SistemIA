            ...
            try
            {
                ...
                if (node != null)
                {
                    ...
                    try
                    {
                        ...
                        // GUARDAR ZIP BASE64 SOLO (para comparar con SistemIA)
                        string zipBase64Path = @"C:\nextsys - GLP\sifen_zip_base64.txt";
                        File.WriteAllText(zipBase64Path, base64String);
                        Log("ZIP Base64 guardado en: " + zipBase64Path);
                        
                        // GUARDAR EL ZIP COMO ARCHIVO BINARIO
                        string zipFilePath = @"C:\nextsys - GLP\sifen_enviado.zip";
                        byte[] zipBytes = Convert.FromBase64String(base64String);
                        File.WriteAllBytes(zipFilePath, zipBytes);
                        Log("ZIP binario guardado en: " + zipFilePath);
                        
                        // GUARDAR EL XML QUE VA DENTRO DEL ZIP (doc.OuterXml sin envolver en rLoteDE)
                        string xmlInternoPath = @"C:\nextsys - GLP\sifen_xml_interno.txt";
                        File.WriteAllText(xmlInternoPath, doc.OuterXml);
                        Log("XML interno guardado en: " + xmlInternoPath);
                    }

...
{
    /// <inheritdoc />
    public partial class Agregar_Descuentos_Sistema : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            // ========== ComprasDetalles - PrecioMinisterio (idempotente) ==========
            migrationBuilder.Sql(@"
        ...
        {
            migrationBuilder.Sql(@"IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID(N'Productos') AND name = 'PermiteDescuento') ALTER TABLE [Productos] DROP COLUMN [PermiteDescuento];");
            migrationBuilder.Sql(@"IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID(N'Productos') AND name = 'DescuentoMaximoProducto') ALTER TABLE [Productos] DROP COLUMN [DescuentoMaximoProducto];");
            migrationBuilder.Sql(@"IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID(N'ConfiguracionSistema') AND name = 'PermitirVenderConDescuento') ALTER TABLE [ConfiguracionSistema] DROP COLUMN [PermitirVenderConDescuento];");
            migrationBuilder.Sql(@"IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID(N'ConfiguracionSistema') AND name = 'PorcentajeDescuentoMaximo') ALTER TABLE [ConfiguracionSistema] DROP COLUMN [PorcentajeDescuentoMaximo];");
            migrationBuilder.Sql(@"IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID(N'VentasDetalles') AND name = 'PrecioMinisterio') ALTER TABLE [VentasDetalles] DROP COLUMN [PrecioMinisterio];");
            migrationBuilder.Sql(@"IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID(N'VentasDetalles') AND name = 'PorcentajeDescuento') ALTER TABLE [VentasDetalles] DROP COLUMN [PorcentajeDescuento];");
            migrationBuilder.Sql(@"IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID(N'ComprasDetalles') AND name = 'PrecioMinisterio') ALTER TABLE [ComprasDetalles] DROP COLUMN [PrecioMinisterio];");
        }
    }
}
